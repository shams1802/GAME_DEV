...
# \- Drag the \*\*SPML\_male.fbx\*\* model from the `Assets/Models` folder into the \*\*Hierarchy\*\* window.  
...
# &nbsp;  - \*\*Character Root\*\* → drag the main \*\*SPML\_male\*\* GameObject.  

# &nbsp;  - \*\*Root Bone For Slider\*\* → expand `SPML\_male`, drag the `m\_avg\_root` bone.  
...
# &nbsp; - Click `+` → drag \*\*PoseManager\*\* into slot.  
